...
exports.crearReferencia = async (req, res) => {
  ...
  try {
    ...
    // Verificar que la referencia no exista ya
    const referenciaExistente = await Referido.findOne({
      where: { cliente_id, referido_id }
    });
    ...
    // Crear la referencia
    const nuevaReferencia = await Referido.create({
      cliente_id,
      referido_id,
    });
    ...
    res.send({ message: "Referencia creada con éxito.", nuevaReferencia });
  } catch (err) {
    res.status(500).send({
